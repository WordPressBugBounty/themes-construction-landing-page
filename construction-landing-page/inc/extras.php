            ...
            <div class="header-block">
                <div class="container">
                    ...
                    <div class="page-header">
                        ...
                        <?php if (is_archive() ) the_archive_description( '<div class="taxonomy-description">', '</div>' ); ?>
                    </div>
...
function construction_landing_page_fonts_url() {
    ...
    */
    ...
    $big_shoulders = _x( 'on', 'Big Shoulders Display: on or off', 'construction-landing-page' );
    $outfit        = _x( 'on', 'Outfit: on or off', 'construction-landing-page' );
    
    $font_families = array();

    if( 'off' !== $pt_sans && ( $my_theme['Name'] !== 'Construction Builders') ){
        $font_families[] = 'PT Sans:400,400italic,700italic,700';
    }

    if( ( $theme_style == 'modern' || $theme_style_cb == 'modern' ) && 'off' !== $big_shoulders ){
        $font_families[] = 'Big Shoulders Display:400,400italic,700italic,700';
    }

    if( ( $theme_style == 'modern' || $theme_style_cb == 'modern' ) && 'off' !== $outfit ){
        $font_families[] = 'Outfit:300,400,400italic,700italic,700';
    }
...
if( ! function_exists( 'construction_landing_page_load_preload_local_fonts') ) :
/**
 ...
 */
function construction_landing_page_load_preload_local_fonts( $url, $format = 'woff2' ) {

    // Check if cached font files data preset present or not. Basically avoiding 'construction_landing_page_WebFont_Loader' class rendering.
    $local_font_files = get_site_option( 'construction_landing_page_local_font_files', false );

    if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
        $font_format = apply_filters( 'construction_landing_page_local_google_fonts_format', $format );
    ...
    $font = construction_landing_page_webfont_loader_instance( $url );
    $font->set_font_format( $format );
    $font->preload_local_fonts();
}
endif;

if( ! function_exists( 'construction_landing_page_flush_local_google_fonts' ) ){
    /**
     * Ajax Callback for flushing the local font
     */
    function construction_landing_page_flush_local_google_fonts() {

        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'construction_landing_page_flush_fonts_nonce' ) ) {
        ...
        $WebFontLoader = new Construction_Landing_Page_WebFont_Loader();
...
add_action( 'wp_ajax_flush_local_google_fonts', 'construction_landing_page_flush_local_google_fonts' );